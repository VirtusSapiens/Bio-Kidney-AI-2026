...
# **A Multi-Layer Geometric Digital Twin of the Human Kidney: Anatomical Fidelity and Calibrated Reduced-Order Feasibility Checks**
...
March 2026 · **v4 revision (oxygen module withdrawn; reproducibility added), July 2026**
...
> **Version note (v4).** This version **withdraws the oxygen-transport module**. An internal audit found that its solver diverges numerically and that the oxygen metrics reported in earlier versions were an artifact of NaN-masking aggregators (Section 4.8); the module is placed in quarantine and **no oxygen figure is reported here**. **The geometric contribution is unaffected.** A dependency trace confirmed that no geometric module consumes the oxygen field: the vascular generator (v7/v8), the population morphometry, and the domain, nephron, arterial (Murray α = 3.0), venous, collecting, and calyceal layers (Layers 0–4) are all computed independently of oxygen. The hypoxia→CCO feedback suggested by the filename `senyal_hipoxia_para_cco.csv` does not exist in the code — that signal is written but never read. Relative to v3, this version also adds explicit git versioning of the geometric layers and a reproducibility section (Section 3.7); the remaining text is unchanged except for the removal of oxygen claims and the corresponding epistemic-tier wording.

---

## Abstract

Chronic kidney disease affects over 850 million people worldwide, and computational tools that describe the geometry of a bioprinted kidney prior to fabrication remain scarce and fragmented. Here we present Bio-Kidney AI 2026, an open, reproducible framework whose **validated contribution is the anatomical and geometric fidelity of a multi-layer digital twin** of the renal vasculature and collecting system, built on modest hardware. The vascular generator (Constrained Constructive Optimization, CCO v8) synthesizes 1,902 hierarchical segments with 100% Murray's Law compliance (α = 3.0) across 915 bifurcations, and a Beta(3, 1.2) radial distribution places 63.1% of demand points in the cortical zone, consistent with human nephron distribution. A two-pass **calibrated** Poiseuille model resolves the failure of naive pressure computation in geometrically compressed trees, producing a monotonic, physiologically ordered pressure cascade with terminal pressures of 58.6 ± 13.4 mmHg. The functional modules — glomerular filtration (Starling–Deen), Co-SWIFT bioprinting optimization (Herschel–Bulkley + MOPSO), iPSC differentiation kinetics, and tubular reabsorption — are presented explicitly as **calibrated reduced-order feasibility checks and design-space exploration, not physiological predictions**. Accordingly, the bilateral figure of ~115 mL/min is reported as a **feasibility check under a calibrated pressure scale (K_f from literature × a calibrated Starling driving force), not a geometric prediction of GFR**; and design targets for cell viability, phenotypic purity, and reabsorption efficiency are cited from the literature rather than claimed as validated outputs. The pipeline executes in under 60 s on consumer hardware (16 GB RAM, Ubuntu Linux) and is released as an open-source Python package. The framework thus contributes a reproducible, honestly-tiered geometric foundation — an anatomically-anchored geometric core with clearly-labeled reduced-order engineering estimates — for pre-fabrication analysis in organ bioengineering.
...
A key limitation of the v7 baseline was that demand seed points were uniformly distributed within the renal ellipsoid, producing homogeneous vascular density across cortex and medulla. This does not reflect human renal anatomy, where approximately 85% of glomeruli reside in the cortical zone [8].
...
where P<sub>aorta</sub> = 100 mmHg is the renal artery inlet pressure and P<sub>target</sub> = 58.0 mmHg is the target mean glomerular capillary pressure, derived from the Starling equilibrium condition: at P<sub>gc</sub> = 58 mmHg, ΔP<sub>Starling</sub> = (58 − 15) − (28 − 0) = 15 mmHg, yielding a predicted single-kidney GFR of 3.7 × 15 = 55.5 mL/min within the physiological range [6]. This 55.5 mL/min is the GFR implied by the *target* pressure (P<sub>gc</sub> = 58.0 mmHg); the *achieved* mean terminal pressure of 58.6 mmHg yields ~58 mL/min per kidney (Section 3.3). The two figures are not inconsistent — one is the calibration target, the other the calibrated result. Scaled pressure drops k · ΔP<sub>raw</sub> were then propagated by breadth-first traversal from root to leaves. A physiological floor of 43 mmHg (the minimum pressure for net filtration: P<sub>Bowman</sub> + π<sub>gc</sub> = 15 + 28 = 43 mmHg) was applied exclusively to terminal nodes.
...
Spatial coverage queries, which dominate the computational cost of the adaptive growth phase, were accelerated by replacing the O(N·M) brute-force distance computation of v7 with a cKDTree implementation from SciPy [9], reducing coverage assessment to O(N log N). Nearest-active-node queries were similarly accelerated via k-nearest-neighbor retrieval (k = 20) from the same spatial index.
...
### 2.3 Co-SWIFT Bioprinting Optimization
...
### 2.4 Glomerular Filtration

Glomerular filtration was modeled with a correctly-implemented Starling–Deen equation along the glomerular capillary; we treat it as a **calibrated reduced-order feasibility check, not an emergent prediction**. The module operates in two clearly-separated modes. In **standalone mode**, terminal pressures from the CCO v7 tree (1,000 demand points) are used as glomerular inlet pressures and scaled to a reference population of 1,000,000 glomeruli [Bertram et al., 2011], yielding a **standalone module estimate of 82 mL/min/1.73m²** (filtration fraction 10.4%) obtained under a *synthetic* pressure model — i.e., an input constant for comparison, not a value produced by the coupled system. The glomerular filtration module reads the legacy CCO v7 tree directly (the v8 twin geometry described in Section 2.1 is not yet wired to the simulators). As a coarse consistency check, the literature K_f multiplied by the mean net Starling pressure (ΔP ≈ 15.6 mmHg, from the mean terminal pressure of 58.6 mmHg) yields an order-of-magnitude bilateral figure of ~115 mL/min — a hand-computed check, not the output of an integrated solver. **We fix K_f = 3.7 mL/min/mmHg for all reported results** [6]; a second value (K_f = 4.1) exists in an exploratory `_G` variant of the module and was *not* used in any figure or number in this manuscript. Because both the pressure scale (via k) and K_f are set from calibration/literature rather than derived from tissue geometry, the resulting bilateral figure (Section 3.3) is a **feasibility check under a calibrated scale, not a geometric prediction of GFR**; net Starling pressure was confirmed positive across all terminal nodes only in the sense that the calibration was chosen to keep it so.

<div style="page-break-after: always;"></div>

### 2.5 Tubular Reabsorption
...
The terminal arterial pressure field is an output of the CCO v8 geometry generator: it is read from the exported `arbol_vascular_cco_v8.csv` (1,902 segments, `presion_salida` column), produced by `generador_cco_v8.py` under its two-pass calibrated Poiseuille pass. Across the 579 arterial terminal nodes it is 58.6 ± 13.4 mmHg (spatial standard deviation; range 43.0–92.1 mmHg), reported as descriptive dispersion and not propagated as an uncertainty on the aggregate GFR. The global calibration factor was k = 0.0132 (the raw Poiseuille resistances required downscaling by approximately two orders of magnitude to match physiological pressure drops). This field is produced by the geometry generator alone: the glomerular-filtration module (Section 2.4) is a separate artefact that runs on the legacy CCO v7 tree — which carries no terminal-pressure column — and does not consume this v8 field. Consequently the bilateral ~115 mL/min reported in Section 3.3 is a by-hand arithmetic check that takes the mean 58.6 mmHg as an input constant (literature K_f × ΔP), not the output of an integrated generator→filtration→GFR chain. Pressure decreased monotonically across hierarchical levels:
...
> GFR<sub>single kidney</sub> = K<sub>f</sub> × ΔP<sub>Starling</sub> = 3.7 × 15.6 = ~58 mL/min

> **GFR<sub>bilateral</sub> = ~115 mL/min**

We emphasize that this figure is a **calibrated feasibility check, not a geometric prediction**: both the pressure scale (via k = 0.0132) and K_f (3.7, from literature) are set by calibration, so ~115 mL/min is what a literature K_f applied by hand to a pressure field *calibrated to physiological targets* gives — by construction it lands in range, and this should not be read as the geometry emergently producing a normal GFR. Its value is as a **consistency check** that the calibrated hemodynamics do not preclude filtration. The v7 baseline, which relied on a uniform demand distribution and lacked an explicit hemodynamic model, produced pressure estimates that were either undefined (no Poiseuille assignment) or physiologically implausible when a naive Poiseuille model was applied (terminal pressures collapsing to 20 mmHg, yielding zero net filtration). The 82 mL/min/1.73m² figure is an **input constant** from the standalone module under a synthetic pressure model (Section 2.4), not a coupled-pipeline output, and it is internally inconsistent with the glomerular sub-models (which point to ~62.5 mL/min); it is retained only to document the v7→v8 methodological change in the pressure model.

All 1,905 nodes (905 arterial, 927 venous, 73 collecting) — the 1,902 segments plus one root node per system (arterial, venous, collecting), so nodes = segments + 3 — were confirmed to lie within the renal ellipsoid boundary (100.0% containment).

![](images/image_f02d63.png)

**Figure 3. Comparative glomerular filtration rate: CCO v7 baseline vs. v8 calibrated model.** **(A)** v7 baseline: uniform demand (1,000 points), synthetic pressure model, GFR approximately 82 mL/min/1.73 m² (CKD Stage 2, ~70% of normal). Under naive Poiseuille, terminal pressures collapsed to 20 mmHg (GFR = 0, red dashed line). **(B)** v8 calibrated: cortically-biased demand (1,300 points, Beta(3, 1.2)), two-pass Poiseuille, bilateral figure ~115 mL/min shown as a feasibility check under a calibrated scale (K_f from literature × calibrated Starling force — not a predicted GFR; green shaded band: normal range 100–125 mL/min). Horizontal dashed lines: 90 mL/min (CKD 1/2 boundary), 60 mL/min (CKD 2/3 boundary). Inset: Starling decomposition — P<sub>gc</sub> = 58.6, P<sub>Bowman</sub> = 15.0, π<sub>gc</sub> = 28.0, ΔP<sub>net</sub> = 15.6 mmHg. No error bars are drawn in this figure; the ±13.4 mmHg reported elsewhere is the spatial standard deviation of terminal *pressure* across arterial terminals, not a dispersion on the GFR estimate. The 40% GFR increase is attributable to the hemodynamic model, not filtration parameters.

### 3.4 iPSC Differentiation
...
Bio-Kidney AI 2026 demonstrates that a reproducible, multi-layer **geometric** digital twin of the kidney — anatomically anchored and Murray-compliant — can be built and audited on consumer hardware. The framework's validated contribution is geometric and field-level fidelity, not physiological function: the functional modules are calibrated reduced-order feasibility checks, and we do not claim a complete in-silico validation of a functional organ.

The principal *methodological* outcome is the calibrated hemodynamic field, not a GFR number. Applying a literature K_f to the mean of the two-pass–calibrated CCO v8 terminal pressure field (mean glomerular capillary pressure 58.6 mmHg; net Starling driving force 15.6 mmHg) gives, by hand, a bilateral figure of ~115 mL/min — an arithmetic feasibility check that takes 58.6 mmHg as an input constant, not a value returned by the glomerular-filtration module (which runs on the legacy v7 tree and does not consume the v8 field). This is reported as a **feasibility check under a calibrated scale**: because both the pressure scale (k) and K_f are set by calibration/literature, the figure lands in range by construction and must not be read as the geometry emergently predicting a normal GFR. The genuine advance from v7 to v8 is that a physiologically-ordered pressure cascade — rather than the undefined or collapsing pressures of the naive model — makes such a check *possible and self-consistent*, which is the useful engineering statement.
...
| GFR (bilateral) | 0 or ~82 mL/min* | ~115 mL/min | Feasibility check, in-range by calibration** |
| Coverage search | O(N·M) brute force | O(N log N) cKDTree | ~100× speedup |
| Execution time (16 GB) | ~3 min | <60 s | ~3× faster |

*v7 GFR of 82 mL/min was obtained using a synthetic pressure distribution, not derived from the vascular tree geometry. **The v8 bilateral figure (~115 mL/min) is a feasibility check produced by a literature K_f (3.7) applied to a pressure field calibrated to physiological targets (k = 0.0132); it lands in range by construction and is not an emergent geometric prediction of GFR (Sections 2.4, 3.3).
...
**Reduced-order module status.** Consistent with the tiering above, the glomerular-filtration and bioprinting modules are calibrated reduced-order checks; the tubular-reabsorption module is in quarantine pending implementation repair (Sections 2.5, 3.6); and the cellular-differentiation module is illustrative. These are labeled as such throughout and none is presented as a validated functional prediction.
...
### 4.8 Oxygen Transport — Withdrawn from This Version

The oxygen-transport module that appeared in previous versions is withdrawn and is not reported here. An internal audit established the following:

**Numerical divergence.** The reaction–diffusion solver applies a vectorized Jacobi update with over-relaxation (ω = 1.6). Jacobi over-relaxation is unstable for ω > 1, so the field grows without bound into overflow and NaN across approximately 98% of the domain. The metrics reported in earlier versions were an artifact of NaN-safe aggregators (mean/minimum operators that ignore NaN), which masked the failure rather than surfacing it.

**Source-term contamination.** The urinary collecting system (renal pelvis and calyces) was admitted into the oxygen source term as a vascular Dirichlet boundary condition, injecting a blood-like partial pressure over a non-vascular structure.

**A scale limit, not merely a bug.** By design, the vascular graph lacks capillary-scale resolution: terminal segments are on the order of 0.6 mm, whereas the oxygen diffusion length is on the order of 150 µm. Even with a convergent solver, this twin cannot support a physiologically valid hypoxic fraction — it would measure the *absence of capillaries in the model*, not the oxygenation of tissue.

**Conclusion.** Oxygen transport is withdrawn and reclassified as future work, conditional on capillary-scale resolution. Consistent with Section 3.7, no geometric layer depends on the oxygen field, so this withdrawal does not affect any geometric or hemodynamic result.
...
This work presents Bio-Kidney AI 2026, a multi-scale computational framework whose validated contribution is a reproducible, multi-layer **geometric** digital twin of the kidney. We make no claim of a complete in-silico validation of a functional organ; the functional modules are explicitly-labeled calibrated reduced-order feasibility checks and illustrative estimates.

The defensible results are geometric: 100% Murray's Law compliance (α = 3.0) across 1,902 segments with a cortically-enriched, anatomically-anchored architecture; and a monotonic, physiologically-ordered pressure cascade with terminal pressures of 58.6 ± 13.4 mmHg from a two-pass–calibrated Poiseuille model. The filtration figure of ~115 mL/min is reported as a feasibility check under a calibrated scale, not as a predicted GFR; and cell-viability, phenotypic-purity, and reabsorption targets are cited from the literature as design objectives, not as validated outputs. The tubular-reabsorption module is in quarantine pending repair.
...
[8] Bertram, J.F., et al. (2011). Human nephron number: implications for health and disease. Pediatric Nephrology, 26(9), 1529-1533.

[9] Virtanen, P., et al. (2020). SciPy 1.0: fundamental algorithms for scientific computing in Python. Nature Methods, 17(3), 261-272.

[10] Guyton, A.C., Hall, J.E. (2020). Textbook of Medical Physiology, 14th ed. Elsevier. Chapter 26: Urine Formation by the Kidneys.

[11] Nordsletten, D.A., et al. (2006). Structural morphology of renal vasculature. American Journal of Physiology — Heart and Circulatory Physiology, 291(1), H296-H309.

[12] Brenner, B.M., Troy, J.L., Daugharty, T.M. (1971). The dynamics of glomerular ultrafiltration in the rat. Journal of Clinical Investigation, 50(8), 1776-1780.
