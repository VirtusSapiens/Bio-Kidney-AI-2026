...
# Ajustadas para reflejar protocolos Takasato 2015 / Freedman 2015:
# diferenciación rápida y agresiva, apoptosis iPSC alta tras señalización
k = {
    # Tasas de diferenciación (día⁻¹)
    "k_iPSC_to_IM":   1.20,
    "k_IM_to_NPC":    0.90,
    "k_NPC_to_PODO":  0.85,   # alta: GDNF es el inductor dominante de podocitos
    "k_NPC_to_PCT":   0.70,   # media-alta: FGF → PCT
    "k_NPC_to_IEC":   0.55,   # media: FGF9 → iECs (menor eficiencia)
    # Proliferación — baja en iPSC tras activación Wnt
    "p_iPSC":         0.10,
    "p_NPC":          0.20,
    "p_PODO":         0.08,
    "p_PCT":          0.10,
    "p_IEC":          0.12,
    # Apoptosis — iPSC alta (selección negativa indiferenciadas)
    "d_iPSC":         0.55,   # eliminación agresiva iPSC residuales
    "d_IM":           0.12,
    "d_NPC":          0.05,
    "d_PODO":         0.03,
    "d_PCT":          0.03,
...
def odes(t, y, doses=None):
    ...
    # Tasas efectivas — cada linaje responde a su factor específico
    # con inhibición cruzada: cuando un linaje domina, suprime a los otros
    total_terminal = max(podo + pct + iec, 1e-12)
    f_podo = podo / total_terminal   # fracción competitiva
    f_pct  = pct  / total_terminal
    f_iec  = iec  / total_terminal

    # Inhibición lateral (Notch-like): el linaje dominante frena a los demás
    inh_podo = 1.0 / (1.0 + 4.0 * (f_pct + f_iec))
    inh_pct  = 1.0 / (1.0 + 4.0 * (f_podo + f_iec))
    inh_iec  = 1.0 / (1.0 + 4.0 * (f_podo + f_pct))

    diff_iPSC_IM = k["k_iPSC_to_IM"]  * wnt  * ipsc
    diff_IM_NPC  = k["k_IM_to_NPC"]   * (bmp + ra) * 0.5 * im
    # Podocitos: exclusivamente GDNF (Takasato 2015)
    diff_NPC_PODO = k["k_NPC_to_PODO"] * gdnf * (1.0 - fgf * 0.6) * inh_podo * npc
    # PCT: señal mixta pero dominada por FGF en ausencia de GDNF
    diff_NPC_PCT  = k["k_NPC_to_PCT"]  * fgf  * (1.0 - gdnf * 0.5) * inh_pct  * npc
    # iECs: exclusivamente FGF9 (Freedman 2015)
    diff_NPC_IEC  = k["k_NPC_to_IEC"]  * fgf  * (1.0 - gdnf * 0.4) * inh_iec  * npc
...
def compute_metrics(t, y):
    ipsc, im, npc, podo, pct, iec = y
    total_all = ipsc + im + npc + podo + pct + iec
    total_all = np.where(total_all < 1e-12, 1e-12, total_all)

    # Fracción global (para riesgo teratoma y gráficas de densidad)
    fracs_global = {
        "iPSC":  ipsc / total_all,
        "IM":    im   / total_all,
        "NPC":   npc  / total_all,
    ...
    # Pureza fenotípica: fracción DENTRO del grupo terminal (PODO+PCT+IEC)
    # — así se mide en flow cytometry en laboratorio
    total_terminal = podo + pct + iec
    total_terminal = np.where(total_terminal < 1e-12, 1e-12, total_terminal)
    fracs_purity = {
        "PODO": podo / total_terminal,
        "PCT":  pct  / total_terminal,
        "IEC":  iec  / total_terminal,
    }

    # Teratoma ≈ fracción iPSC sobre el total (riesgo clínico real)
    teratoma_risk = fracs_global["iPSC"]

    # Ventana óptima: pureza ≥ 95% dentro del grupo terminal
    windows = {}
    for linaje in ["PODO", "PCT", "IEC"]:
        f    = fracs_purity[linaje]
        mask = f >= PURITY_MIN
        idx  = np.where(mask)[0]
        if len(idx) > 0:
    ...
    # Índice de pureza fenotípica (área bajo curva ≥95% / área total del linaje)
    purity_indices = {}
    for linaje in ["PODO", "PCT", "IEC"]:
        f = fracs_purity[linaje]
    ...
    # Exponer fracs_purity en fracs para los paneles de pureza
    fracs = fracs_global.copy()
    fracs["PODO_purity"] = fracs_purity["PODO"]
    fracs["PCT_purity"]  = fracs_purity["PCT"]
    fracs["IEC_purity"]  = fracs_purity["IEC"]
...
def plot_all(t, y, fracs, teratoma_risk, windows, purity_indices):
    ...
    for span, col, lbl in [((0,5),"#58A6FF20","Wnt"),((5,12),"#BC8CFF20","BMP7+RA"),((12,21),"#3FB95020","GDNF+FGF")]:
        ...
        ax1.text((span[0]+span[1])/2, 0.97, lbl,
    ...
    ax2.set_title("Pureza fenotípica — dentro de linajes terminales (%)", color=TEXT_MAIN, fontsize=10)
    for linaje, col, key in [
        ("PODO", C_PODO, "PODO_purity"),
        ("PCT",  C_PCT,  "PCT_purity"),
        ("IEC",  C_IEC,  "IEC_purity"),
        ("iPSC (global)", C_TERA, "iPSC"),
    ]:
        arr = fracs[key] if key in fracs else fracs[linaje]
    ...
    # Falla 5 corregida: 3 fill_between en lugar de 3000 iteraciones
    mask_high = risk_pct > 5.0
    mask_mid  = (risk_pct > 1.0) & ~mask_high
    mask_low  = risk_pct <= 1.0
    for mask, col in [(mask_high, RED), (mask_mid, ORANGE), (mask_low, GREEN)]:
        ax3.fill_between(t, risk_pct, where=mask, color=col, alpha=0.65)
    ax3.plot(t, risk_pct, color=TEXT_DIM, lw=0.8, alpha=0.5)
    ax3.axhline(5.0, color=RED,    lw=1.2, ls="--", label="Riesgo alto (>5%)")
    ax3.axhline(1.0, color=ORANGE, lw=1.2, ls="--", label="Riesgo moderado (>1%)")
    # Falla 2 corregida: usar enumerate en lugar de variable i suelta
    label_y_levels = [0.80, 0.65, 0.50]
    for idx_w, (linaje, (t0, t1)) in enumerate(windows.items()):
        if t0 is not None:
            ax3.axvspan(t0, t1, alpha=0.1, color=GREEN)
            ylim_top = ax3.get_ylim()[1] if ax3.get_ylim()[1] > 0 else 10
            ax3.text((t0+t1)/2, ylim_top * label_y_levels[idx_w],
...
if __name__ == "__main__":
    ...
    plt.tight_layout(rect=[0, 0.02, 1, 0.97])   # falla 4 corregida: antes de savefig
